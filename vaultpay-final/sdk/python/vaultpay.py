"""
╔══════════════════════════════════════════════════╗
║         VaultPay Python SDK v1.0                 ║
║   Secure Payment Gateway for Python Developers   ║
╚══════════════════════════════════════════════════╝

Installation:
    Copy this file to your project OR:
    pip install vaultpay   (when published)

Usage:
    from vaultpay import VaultPay

    vp = VaultPay("vp_live_YOUR_API_KEY")
...
SDK_VERSION = "1.0.0"
DEFAULT_BASE_URL = "https://api.vaultpay.io/api/v1"
DEFAULT_TIMEOUT = 30


# ─── Exceptions ───────────────────────────────────────────────────────────────

class VaultPayError(Exception):
    """Raised when the VaultPay API returns an error."""
    ...
    def __repr__(self):
        return f"VaultPayError(code={self.code!r}, message={str(self)!r}, status={self.status_code})"


class VaultPayAuthError(VaultPayError):
    """Raised for authentication failures."""
    pass


class VaultPayValidationError(VaultPayError):
    """Raised for invalid request parameters."""
    pass


class VaultPayNotFoundError(VaultPayError):
    """Raised when a resource is not found."""
    pass


class VaultPayNetworkError(VaultPayError):
...
class VaultPay:
    """
    VaultPay payment gateway client.

    Args:
        api_key: Your VaultPay API key (vp_live_... or vp_test_...)
        base_url: Override the API base URL (useful for self-hosted)
        timeout: Request timeout in seconds (default: 30)

    Example:
        vp = VaultPay("vp_live_abc123...")
    ...
    def __init__(
        ...
        if not api_key or not isinstance(api_key, str):
            raise VaultPayError("api_key is required", code="MISSING_API_KEY")
    ...
    def _request(
        ...
        """
        Make an authenticated request to the VaultPay API.

        Raises VaultPayError subclasses on failure.
        ...
        headers = {
            ...
            "X-VaultPay-Key": self._api_key,
            "X-VaultPay-SDK": f"python/{SDK_VERSION}",
            "User-Agent": f"VaultPay-SDK-Python/{SDK_VERSION}",
        }
        if payload:
        ...
        try:
            ...
            if exc.code == 401:
                raise VaultPayAuthError(msg, code=code, status_code=exc.code, raw=parsed)
            if exc.code == 422 or exc.code == 400:
                raise VaultPayValidationError(msg, code=code, status_code=exc.code, raw=parsed)
            if exc.code == 404:
                raise VaultPayNotFoundError(msg, code=code, status_code=exc.code, raw=parsed)

            raise VaultPayError(msg, code=code, status_code=exc.code, raw=parsed)
    ...
    def verify_webhook_signature(
        ...
        Args:
            ...
            signature: The X-VaultPay-Signature header value
        ...
        Example (Django):
            body = request.body
            sig  = request.META.get("HTTP_X_VAULTPAY_SIGNATURE", "")
            if not VaultPay.verify_webhook_signature(body, sig, WEBHOOK_SECRET):
                return HttpResponse(status=401)

        Example (Flask):
            body = request.get_data()
            sig  = request.headers.get("X-VaultPay-Signature", "")
            if not VaultPay.verify_webhook_signature(body, sig, WEBHOOK_SECRET):
                abort(401)
        """
...
class PaymentsResource:
    """Manage payment orders via VaultPay API."""

    def __init__(self, client: VaultPay):
        self._client = client
    ...
    def create(
        ...
        Raises:
            VaultPayValidationError: If parameters are invalid
            VaultPayAuthError: If API key is invalid
        ...
        """
        if not order_id:
            raise VaultPayError("order_id is required", code="MISSING_PARAM")
        ...
        if not customer or not customer.get("email"):
            raise VaultPayError("customer.email is required", code="MISSING_PARAM")
    ...
    def fetch(self, payment_id: str) -> Dict[str, Any]:
        ...
        Args:
            payment_id: The VaultPay payment ID (pay_...)

        Returns:
            Full payment object including status, customer, qr_code, etc.
        """
        if not payment_id:
            raise VaultPayError("payment_id is required", code="MISSING_PARAM")
...
class QRResource:
    """Generate QR codes via VaultPay API."""
    ...
    def generate(
        ...
        """
        if not text:
            raise VaultPayError("text is required", code="MISSING_PARAM")
...
class SMSResource:
    ...
    def parse(
        ...
        if not sms_text or len(sms_text.strip()) < 10:
            raise VaultPayError("sms_text must be at least 10 characters", code="MISSING_PARAM")
...
def verify_webhook(raw_body: bytes, signature: str, secret: str) -> bool:
    """
    Module-level shorthand for VaultPay.verify_webhook_signature().
    """
    return VaultPay.verify_webhook_signature(raw_body, signature, secret)
...
class DjangoWebhookMixin:
    """
    Django view mixin for handling VaultPay webhooks.

    Usage:
        from vaultpay import DjangoWebhookMixin
        from django.views import View

        class VaultPayWebhook(DjangoWebhookMixin, View):
            webhook_secret = settings.VAULTPAY_WEBHOOK_SECRET
    ...
    def post(self, request, *args, **kwargs):
        ...
        signature = request.META.get("HTTP_X_VAULTPAY_SIGNATURE", "")
        if not VaultPay.verify_webhook_signature(request.body, signature, self.webhook_secret):
...
if __name__ == "__main__":
    ...
    key = os.environ.get("VAULTPAY_API_KEY", "")
    base_url = os.environ.get("VAULTPAY_BASE_URL", "http://localhost:5000/api/v1")

    if not key:
        print("Set VAULTPAY_API_KEY environment variable to test")
        print("  export VAULTPAY_API_KEY=vp_test_your_key_here")
        print("  export VAULTPAY_BASE_URL=http://localhost:5000/api/v1")
        sys.exit(1)

    print(f"\n🔑 Testing VaultPay Python SDK v{SDK_VERSION}")
    print(f"   Key: {key[:16]}...")
    print(f"   URL: {base_url}\n")

    vp = VaultPay(key, base_url=base_url)
