/**
 * VaultPay — Shared Frontend Utilities
...
const VP = {
  ...
  get baseUrl() {
    return (window.VAULTPAY_API_URL || '').replace(/\/$/, '');
  },
  ...
  async request(method, path, body = null, useApiKey = false) {
    ...
    if (useApiKey) {
      headers['X-VaultPay-Key'] = this.testKey;
    } else if (this.token) {
...
const UI = {
  ...
  toast(message, duration = 2500) {
    ...
    setTimeout(() => el.classList.add('vp-toast-show'), 10);
    setTimeout(() => {
      el.classList.remove('vp-toast-show');
...
(function injectSharedStyles() {
  ...
  style.textContent = `
    .vp-alert{padding:12px 16px;border-radius:10px;font-size:14px;font-weight:600;display:none;margin-bottom:16px;}
    .vp-alert-success{background:#d1fae5;color:#065f46;border:1px solid #a7f3d0;}
    .vp-alert-error{background:#fee2e2;color:#991b1b;border:1px solid #fecaca;}
    .vp-alert-warning{background:#fef3c7;color:#92400e;border:1px solid #fde68a;}
    .vp-alert-info{background:#dbeafe;color:#1e40af;border:1px solid #bfdbfe;}
    .vp-toast{position:fixed;bottom:24px;right:24px;background:#1f2937;color:#f9fafb;padding:12px 20px;border-radius:10px;font-size:14px;font-weight:600;z-index:9999;opacity:0;transform:translateY(8px);transition:all 0.25s;box-shadow:0 8px 24px rgba(0,0,0,0.3);}
    .vp-toast-show{opacity:1;transform:translateY(0);}
    .vp-spinner{display:inline-block;width:18px;height:18px;border:2.5px solid rgba(255,255,255,0.3);border-top-color:white;border-radius:50%;animation:vp-spin 0.7s linear infinite;}
