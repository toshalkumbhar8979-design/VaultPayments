'use strict';
/**
 * VaultPay Node.js SDK v2.0
 * Zero dependencies — pure Node.js stdlib
 *
 * Usage:
 *   const VaultPay = require('./vaultpay');
 *   const vp = new VaultPay('vp_live_YOUR_KEY');
...
class VaultPayError extends Error {
...
class VaultPay {
  constructor(apiKey, options = {}) {
    if (!apiKey) throw new VaultPayError('API key is required', 'MISSING_API_KEY');
    if (!/^vp_(live|test)_[a-f0-9]{32}$/.test(apiKey)) {
      throw new VaultPayError('Invalid API key format. Expected vp_live_... or vp_test_...', 'INVALID_KEY_FORMAT');
    }
    this._key      = apiKey;
    ...
    return new Promise((resolve, reject) => {
      ...
      const opts = {
        ...
        headers: {
          ...
          'X-VaultPay-Key': this._key,
          'X-VaultPay-SDK': `node/${SDK_VER}`,
          'User-Agent':     `VaultPay-SDK-Node/${SDK_VER}`,
      ...
      const req = lib.request(opts, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            if (!parsed.success) {
              return reject(new VaultPayError(
            ...
            reject(new VaultPayError('Failed to parse response', 'PARSE_ERROR', res.statusCode, data));
          }
        });
      });

      req.on('timeout', () => { req.destroy(); reject(new VaultPayError('Request timed out', 'TIMEOUT', 408)); });
      req.on('error',  (e) => reject(new VaultPayError(`Network error: ${e.message}`, 'NETWORK_ERROR')));
  ...
  /**
   * Verify a VaultPay webhook signature.
   * ALWAYS call this before trusting webhook data.
   *
   * @param {Buffer|string} rawBody  — raw request body
   * @param {string}        signature — X-VaultPay-Signature header
...
class PaymentsResource {
  ...
  async create({ order_id, amount, currency = 'INR', customer, description = '',
    ...
    if (!order_id)          throw new VaultPayError('order_id is required',         'MISSING_PARAM');
    if (!amount || amount < 100) throw new VaultPayError('amount must be ≥ 100 paise','INVALID_AMOUNT');
    if (!customer?.email)   throw new VaultPayError('customer.email is required',   'MISSING_PARAM');
    if (!customer?.phone)   throw new VaultPayError('customer.phone is required',   'MISSING_PARAM');
...
module.exports = VaultPay;
module.exports.VaultPayError = VaultPayError;
module.exports.default = VaultPay;
